...
int RenderDevice::Initialize(HWND hWnd)
{
	...
	if (m_bFull)
	{
		...
		if (GetMonitorInfo(hMon, &mi))
		{
			...
			int nActualWidth = rcClient.right - rcClient.left;
			int nActualHeight = rcClient.bottom - rcClient.top;

			m_d3dpp.BackBufferWidth = nActualWidth;
			m_d3dpp.BackBufferHeight = nActualHeight;
			m_d3dsdBackBuffer.Width = nActualWidth;
			m_d3dsdBackBuffer.Height = nActualHeight;
			m_dwScreenWidth = nActualWidth;
			m_dwScreenHeight = nActualHeight;
			m_rcWindowClient.right = nActualWidth;
			m_rcWindowClient.bottom = nActualHeight;

			if (nActualWidth > 0 && nActualHeight > 0)
			{
				RenderDevice::r_fWidthRatio = (float)nActualWidth / 800.0f;
				RenderDevice::r_fHeightRatio = (float)nActualHeight / 600.0f;
				RenderDevice::m_fWidthRatio = (float)nActualWidth / 1366.0f;
				RenderDevice::m_fHeightRatio = (float)nActualHeight / 768.0f;
				if ((float)nActualWidth == 800 || (float)nActualWidth == 640 || (float)nActualWidth == 1024 || (float)nActualWidth == 1280) {
					RenderDevice::m_fWidthRatio = (float)nActualWidth / 800.0f;
					RenderDevice::m_fHeightRatio = (float)nActualHeight / 600.0f;
				}
			}
...
void RenderDevice::SetViewPort(int nStartX, int nStartY, int nWidth, int nHeight)
{
	if (m_bBorderlessFullscreen)
	{
		...
		nStartX = 0;
		nStartY = 0;
		if (m_d3dsdBackBuffer.Width > 0 && m_d3dsdBackBuffer.Height > 0)
		{
			nWidth = (int)m_d3dsdBackBuffer.Width;
			nHeight = (int)m_d3dsdBackBuffer.Height;
		}
	}
	...
	if (m_bBorderlessFullscreen)
	{
		static int s_nVpLogCount = 0;
		if (s_nVpLogCount < 20)
		{
			LOG_WRITELOGSTRING("[RenderDevice] SetViewPort borderless: VP=%dx%d BackBuffer=%dx%d Screen=%dx%d\r\n",
				(int)m_viewport.Width, (int)m_viewport.Height,
				(int)m_d3dsdBackBuffer.Width, (int)m_d3dsdBackBuffer.Height,
				(int)m_dwScreenWidth, (int)m_dwScreenHeight);
			s_nVpLogCount++;
		}
	}
...
{
	// Borderless fullscreen: do not change the system display mode.
	...
	m_dwScreenWidth = m_dwCurrScreenX;
	m_dwScreenHeight = m_dwCurrScreenY;
...
{
	// Borderless fullscreen: we never changed the system display mode.
}
...
int RenderDevice::InitVertexShader()
{
	...
	if (m_pVertexShader[0] == nullptr)
	{
		for (int i = 0; i < 8; ++i)
		{
			...
			// [CORREÇÃO] Verificar se D3DXCreateBuffer teve sucesso e se pCode é válido
			if (FAILED(D3DXCreateBuffer(nLength, &pCode)) || !pCode)
			{
				_close(handle);
				return 0;
			}
...
int RenderDevice::InitPixelShader()
{
	...
	if (d3dCaps.PixelShaderVersion >= 0xFFFF0101)
	{
		if (m_pPSEffect[0] == nullptr)
		{
			...
			{
				LPD3DXBUFFER pCode = nullptr;
				// [CORREÇÃO] Aumentar buffer para MAX_PATH para evitar estouro
...
void RenderDevice::RenderRect(float iStartX, float iStartY, float iCX, float iCY, float iDestX, float iDestY, IDirect3DTexture9* pTexture, float fScaleX, float fScaleY)
{
	...
	// [CORREÇÃO] Verificar m_pSprite e pTexture antes de usar
...
void RenderDevice::RenderRectRot(float iStartX, float iStartY, float iCX, float iCY, float iDestX, float iDestY, float nCenX, float nCenY, float fAngle, IDirect3DTexture9* pTexture, float fScaleX, float fScaleY)
{
	...
	// [CORREÇÃO] Verificar m_pSprite antes de usar
	if (pTexture != nullptr && m_pSprite != nullptr)
	{
		m_pSprite->Begin();
...
void RenderDevice::RenderGeomRectImage(GeomControl* ipControl)
{
	if (ipControl->nTextureSetIndex >= 0)
	{
		...
		// [CORREÇÃO] Verificar se pUISet e pTextureCoord são válidos
		if (pUISet == nullptr)
			return;
	...
	if (ipControl != nullptr && ipControl->bVisible == 1)
	{
		...
		case RENDERCTRLTYPE::RENDER_3DOBJ:
			if (ipControl->n3DObjIndex < 737 || ipControl->n3DObjIndex > 739)
			{
				...
				// [CORREÇÃO] Verificar se pMesh é válido antes de usar
				if (!pMesh) break;
			...
			else
			{
				...
				if (pMesh)
				{
					...
					{
						RDLVERTEX* pVertex;
						// [CORREÇÃO] Verificar sucesso do Lock
						if (SUCCEEDED(pMesh->m_pVB->Lock(0, 0, (void**)&pVertex, 0)))
						{
...
void RenderDevice::CaptureScreen()
{
	// Função original de captura de tela (inalterada)
	LPDIRECT3DSURFACE9 pSurface = nullptr;
	if (SUCCEEDED(m_pd3dDevice->CreateOffscreenPlainSurface(
		...
		nullptr)))
	{
		if (SUCCEEDED(m_pd3dDevice->GetFrontBufferData(0, pSurface)))
		{
			char szFileName[MAX_PATH];
			SYSTEMTIME st;
			GetLocalTime(&st);
			sprintf_s(szFileName, "Screenshot_%04d%02d%02d_%02d%02d%02d.bmp",
				st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);

			D3DXSaveSurfaceToFileA(szFileName, D3DXIFF_BMP, pSurface, nullptr, nullptr);
		}
