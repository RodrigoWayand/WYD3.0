	...
	: TMScene()
{
	...
	memset(m_vecMoveToPos, 0, sizeof(m_vecMoveToPos));

	memset(m_pLogoPanels, 0, sizeof(m_pLogoPanels));
	memset(m_pLoginBtns, 0, sizeof(m_pLoginBtns));
	memset(m_pGroupPanel, 0, sizeof(m_pGroupPanel));

	m_pRain = nullptr;
	m_pSnow = nullptr;

	m_pLoginPanel = nullptr;
	m_Logo1 = nullptr;
	m_Logo2 = nullptr;

	m_pLoginPanelText = nullptr;
	m_pLoginIDText = nullptr;
	m_pLoginPasswordText = nullptr;

	m_pSelServerText = nullptr;
	m_pSelChannelText = nullptr;

	m_pEditID = nullptr;
	m_pEditPW = nullptr;

	m_pNServerSelect = nullptr;
	m_pNServerGroupList = nullptr;
	m_pNServerList = nullptr;
...
TMSelectServerScene::~TMSelectServerScene()
{
	...
	{
		auto pSound = g_pSoundManager->GetSoundData(102);

		if (pSound != nullptr && pSound->IsSoundPlaying())
		{
			pSound->Stop();
		}
	}
...
int TMSelectServerScene::InitializeScene()
{
	...
	if (m_Logo1 && m_Logo2)
	{
		if (g_pDevice->m_dwScreenWidth == 800 ||
			g_pDevice->m_dwScreenWidth == 640 ||
			g_pDevice->m_dwScreenWidth == 1024 ||
			g_pDevice->m_dwScreenWidth == 1280)
		{
...
int TMSelectServerScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	case B_LOGIN_OK:
	{
		...
		if (!pLoginOK || !m_pEditID || !m_pEditPW)
			return 1;
		...
		if (!g_pSocketManager ||
			!g_pSocketManager->ConnectServer(
				g_pApp->m_szServerIP,
				TM_CONNECTION_PORT,
				0,
				1124))
		...
		strncpy_s(
			stAccountLogin.AccountName,
			sizeof(stAccountLogin.AccountName),
			pEditID->GetText(),
			_TRUNCATE
		);

		strncpy_s(
			stAccountLogin.AccountPass,
			sizeof(stAccountLogin.AccountPass),
			pEditPassword->GetText(),
			_TRUNCATE
		);
		...
		if (g_pSocketManager)
		{
			g_pSocketManager->SendOneMessage(
				reinterpret_cast<char*>(&stAccountLogin),
				sizeof(MSG_AccountLogin));
		}
	...
	case B_CREATE_ID:
		if (Reconnect == false) {
			Reconnect = true;
			m_pMessagePanel->SetMessage("Reconnect Ativado", 4000);
			m_pMessagePanel->SetVisible(1, 2);
			return 1;
		}
		if (Reconnect == true) {
			Reconnect = false;
			m_pMessagePanel->SetMessage("Reconnect Desativado", 4000);
			m_pMessagePanel->SetVisible(1, 2);
			return 1;
		}
		break;
...
int TMSelectServerScene::FrameMove(unsigned int dwServerTime)
{
	...
	const int camElapsed =
		static_cast<int>(dwServerTime - m_dwStartCamTime);

	if (g_pDevice &&
		g_pCursor &&
		g_pDevice->m_bLoadMeshManager == 1)
	{
	...
	for (int nPerson = 0; nPerson < 50; ++nPerson)
	{
		auto pHuman = m_pCheckHumanList[nPerson];

		if (pHuman)
		{
			pHuman->m_fWantHeight = -4.0f;
		}
	}
...
{
	constexpr int MAX_DEMO_HUMAN = 50;

	for (int nPerson = 0; nPerson < MAX_DEMO_HUMAN; ++nPerson)
	{
	...
	if (fp != nullptr)
	{
		for (int i = 0; i < 50; ++i)
		{
			...
			size_t ret =
				fread(&m_stDemoHuman[i],
					1,
					sizeof(m_stDemoHuman[i]),
					fp);
			if (!ret)
				break;
			...
			sprintf_s(m_pCheckHumanList[i]->m_szName,
	sizeof(m_pCheckHumanList[i]->m_szName),
	"person%d",
	i);
