...
int CMesh::Render(int nBright)
{
    ...
    // Check if weapon rendering should bypass normal mesh render
    ...
    {
        return RenderWeaponSpecial(nPartIndex);
    }

    if (nPartIndex >= 2912 && nPartIndex <= 2923)
    {
        return RenderWeaponSpecial2(nPartIndex);
    }
...
// ---------------------------------------------------------------------------
// RenderWeaponSpecial — weapon common mesh 2889-2909
// ---------------------------------------------------------------------------

bool CMesh::RenderWeaponSpecial(int nPartIndex)
{
    g_pDevice->SetRenderState(D3DRENDERSTATETYPE::D3DRS_ALPHATESTENABLE, 0);
    ...
    char cAlpha = g_pTextureManager->m_stModelTextureList[pMesh->m_nTextureIndex[0]].cAlpha;
    if (cAlpha != 'C')
    {
...
// ---------------------------------------------------------------------------
// RenderWeaponSpecial2 — weapon common mesh 2912-2923
// ---------------------------------------------------------------------------

bool CMesh::RenderWeaponSpecial2(int nPartIndex)
{
    g_pDevice->SetRenderState(D3DRENDERSTATETYPE::D3DRS_ALPHATESTENABLE, 0);
    TMMesh* pMesh = g_pMeshManager->GetCommonMesh(nPartIndex, 0, 20_min);
    if (g_pDevice->m_pd3dDevice->SetStreamSource(0, pMesh->m_pVB, 0, pMesh->m_sizeVertex) < 0)
...
// ---------------------------------------------------------------------------
// AnimateWeaponUV — scroll UVs for weapon effects
// ---------------------------------------------------------------------------

void CMesh::AnimateWeaponUV(TMMesh* pMesh)
{
...
// ---------------------------------------------------------------------------
// RenderMesh — main mesh rendering with multitexture support
// ---------------------------------------------------------------------------
...
int CMesh::RenderMesh(char cAlpha)
{
    ...
    {
        ApplyMultiTextureNone();
    }

    // Render weapon glow parts on hands
    if (m_pParentSkin->m_pOwner
        && (!m_pParentSkin->m_nBoneAniIndex
            || m_pParentSkin->m_nBoneAniIndex == 1
            || m_pParentSkin->m_nBoneAniIndex == 2
    ...
    // No mesh — just cleanup texture stages if multitexture was active
    if (m_sMultiType > 0)
    {
...
// ---------------------------------------------------------------------------
// RenderSkinMesh — render the actual skin mesh with bone palette
// ---------------------------------------------------------------------------

void CMesh::RenderSkinMesh()
{
    ...
    if (g_pDevice->m_pd3dDevice->DrawIndexedPrimitive(D3DPRIMITIVETYPE::D3DPT_TRIANGLELIST, 0, 0,
        m_pMesh->m_AttRange[0].VertexCount, 0, m_numFaces) < 0)
        return;
...
// ---------------------------------------------------------------------------
// RenderMouseOverHighlight — highlight mesh when mouse is over
// ---------------------------------------------------------------------------

void CMesh::RenderMouseOverHighlight()
{
    if (!g_pCurrentScene->m_pMouseOverHuman
        || g_pObjectManager->m_bTvControl
        || g_pCurrentScene->m_pMouseOverHuman != static_cast<TMHuman*>(m_pParentSkin->m_pOwner)
    ...
    for (size_t l = 0; l < m_numPalette; ++l)
    {
        ...
        g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(3 * l + 9, (const float*)&rMatrix, 3);
    }
    ...
    g_pDevice->m_pd3dDevice->DrawIndexedPrimitive(D3DPRIMITIVETYPE::D3DPT_TRIANGLELIST,
        0, 0, m_pMesh->m_AttRange[0].VertexCount, 0, m_numFaces);
...
void CMesh::ApplyMultiTextureLegend4(char cAlpha)
{
    if (m_sMultiType > 12)
        m_sMultiType = 12;
...
void CMesh::ApplyMultiTextureLegend5to8(char cAlpha)
{
    int nBaseIndex = 153;
    if (m_sLegendType == 6) nBaseIndex = 166;
    if (m_sLegendType == 7) nBaseIndex = 246;
    if (m_sLegendType == 8) nBaseIndex = 260;
    if (m_sMultiType > 12)  m_sMultiType = 12;
...
void CMesh::ApplyMultiTextureLegend116to125()
{
    int nBaseIndex = 0;
    switch (m_sLegendType - 116)
    {
    case 0: nBaseIndex = 275; break;
    case 1: nBaseIndex = 288; break;
    case 2: nBaseIndex = 301; break;
    case 3: nBaseIndex = 314; break;
    case 4: nBaseIndex = 327; break;
    case 5: nBaseIndex = 340; break;
    case 6: nBaseIndex = 353; break;
    case 7: nBaseIndex = 366; break;
    case 8: nBaseIndex = 425; break;
    case 9: nBaseIndex = 392; break;
    }
...
void CMesh::ApplyMultiTextureLegend9to12(char cAlpha)
{
    int nBaseIndex = 153;
    if (m_sLegendType == 10) nBaseIndex = 166;
    if (m_sLegendType == 11) nBaseIndex = 246;
    if (m_sLegendType == 12) nBaseIndex = 440;
...
void CMesh::ApplyMultiTextureDefault(char cAlpha)
{
    if (m_bMount)
    {
...
void CMesh::ApplyMultiTextureNone()
{
    g_pDevice->SetTextureStageState(0, D3DTEXTURESTAGESTATETYPE::D3DTSS_COLOROP, 4);
...
int CMesh::LoadMesh(char* file)
{
    ...
    {
        return LoadNewSkinMesh(file) ? 1 : 0;
    }
    else
    {
        return LoadExistingSkinMesh(m_nSkinMeshIndex) ? 1 : 0;
    }
}

bool CMesh::LoadNewSkinMesh(char* file)
{
    ...
    ComputeBoundingSphere(m_pMesh, pVertex, m_pMesh->m_sizeVertex >> 2, m_pMesh->m_AttRange[0].VertexCount);
...
bool CMesh::LoadExistingSkinMesh(int nSkinMeshIndex)
{
    m_pMesh = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].pMesh;
    if (!m_pMesh)
        return false;
    ...
    m_pBoneMatrix = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].m_pBoneMatrix;
    m_dwID = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].m_dwID;
    m_numFaceInflunce = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].m_numFaceInflunce;
    m_numPalette = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].m_numPalette;
    m_dwNames = g_pMeshManager->m_stSkinMeshList[nSkinMeshIndex].m_dwNames;
    m_numFaces = m_pMesh->m_AttRange[0].FaceCount / 3;
    m_bMeshGenerated = 1;
...
void CMesh::ComputeBoundingSphere(TMMesh* pMesh, float* pVertex, int nFloatCount, int nNumVertex)
{
    for (int i = 0; i < nNumVertex - 1; ++i)
    {
        if (pVertex[nFloatCount * i] > pMesh->m_fMaxX)
            pMesh->m_fMaxX = pVertex[nFloatCount * i];
        if (pMesh->m_fMinX > pVertex[nFloatCount * i])
            pMesh->m_fMinX = pVertex[nFloatCount * i];
        if (pVertex[nFloatCount * i + 1] > pMesh->m_fMaxY)
            pMesh->m_fMaxY = pVertex[nFloatCount * i + 1];
        if (pMesh->m_fMinY > pVertex[nFloatCount * i + 1])
            pMesh->m_fMinY = pVertex[nFloatCount * i + 1];
        if (pVertex[nFloatCount * i + 2] > pMesh->m_fMaxZ)
            pMesh->m_fMaxZ = pVertex[nFloatCount * i + 2];
        if (pMesh->m_fMinZ > pVertex[nFloatCount * i + 2])
            pMesh->m_fMinZ = pVertex[nFloatCount * i + 2];
    }

    auto updateRadius = [&](float val) {
        if (fabsf(val) > pMesh->m_fRadius)
            pMesh->m_fRadius = fabsf(val);
        };

    updateRadius(pMesh->m_fMaxX);
    updateRadius(pMesh->m_fMinX);
    updateRadius(pMesh->m_fMaxY);
    updateRadius(pMesh->m_fMinY);
    updateRadius(pMesh->m_fMaxZ);
    updateRadius(pMesh->m_fMinZ);
}

// ---------------------------------------------------------------------------
// InitEffect — initialize swing effects for weapon bones
// ---------------------------------------------------------------------------
...
void CMesh::SetMaterial(char cAlpha)
{
    if (m_pParentSkin != nullptr)
    {
        ...
        g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(8, (const float*)&materials, 1);
        g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(7, (const float*)&ambEmm, 1);
        g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(0, (const float*)&vConst, 1);
        ...
        g_pDevice->m_pd3dDevice->SetVertexShaderConstantF(6, (const float*)&fFog, 1);
    }
}