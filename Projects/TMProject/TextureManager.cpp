...
int TextureManager::GetUITextureIndex(char* szTextureName)
{
	...
	const int nStrLen = (int)strlen(szTextureName);
	if (nStrLen < 3)
		return -1;
	...
	{
		const int nStrLen2 = (int)strlen(m_stUITextureList[i].szFileName);
		if (nStrLen2 < 3)
			continue;

		strncpy(szTemp, szTextureName, sizeof(szTemp) - 1);
		memcpy(&szTemp[nStrLen - 3], &m_stUITextureList[i].szFileName[nStrLen2 - 3], 3);
		szTemp[sizeof(szTemp) - 1] = '\0';

		if (!strcmp(szTemp, m_stUITextureList[i].szFileName))
			return i;
	}
...
int TextureManager::GetEffectTextureIndex(char* szTextureName)
{
	...
	{
		const int nStrLen2 = (int)strlen(m_stEffectTextureList[i].szFileName);
		if (nStrLen2 < 3)
			continue;

		strncpy(szTemp, szTextureName, sizeof(szTemp) - 1);
		memcpy(&szTemp[nStrLen - 3], &m_stEffectTextureList[i].szFileName[nStrLen2 - 3], 3);
		szTemp[sizeof(szTemp) - 1] = '\0';

		if (!strcmp(szTemp, m_stEffectTextureList[i].szFileName))
			return i;
	}
...
int TextureManager::InitRenderTargetTexture()
{
	...
	} // adicionado chave de fechamento do for loop
	return 1;
}
