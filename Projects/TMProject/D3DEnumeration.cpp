...
//-----------------------------------------------------------------------------
// Format info helper functions
//-----------------------------------------------------------------------------

namespace
{
    // ---------------------------------------------------------------------------
    // ColorChannelBits — Returns the number of color channel bits in the specified D3DFORMAT
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // AlphaChannelBits — Returns the number of alpha channel bits in the specified D3DFORMAT
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // DepthBits — Returns the number of depth bits in the specified D3DFORMAT
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // StencilBits — Returns the number of stencil bits in the specified D3DFORMAT
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // SortModesCallback — Used to sort D3DDISPLAYMODEs
    // ---------------------------------------------------------------------------
    int __cdecl SortModesCallback(const void* arg1, const void* arg2)
    {
        D3DDISPLAYMODE* pdm1 = static_cast<D3DDISPLAYMODE*>(const_cast<void*>(arg1));
        D3DDISPLAYMODE* pdm2 = static_cast<D3DDISPLAYMODE*>(const_cast<void*>(arg2));
...
D3DAdapterInfo::~D3DAdapterInfo(void)
{
    ...
    if (pDeviceInfoList != NULL)
    {
        for (UINT idi = 0; idi < pDeviceInfoList->Count(); idi++)
            delete static_cast<D3DDeviceInfo*>(pDeviceInfoList->GetPtr(idi));
...
D3DDeviceInfo::~D3DDeviceInfo(void)
{
    if (pDeviceComboList != NULL)
    {
        for (UINT idc = 0; idc < pDeviceComboList->Count(); idc++)
            delete static_cast<D3DDeviceCombo*>(pDeviceComboList->GetPtr(idc));
...
{
    if (pDepthStencilFormatList != NULL)  delete pDepthStencilFormatList;
    if (pMultiSampleTypeList != NULL)     delete pMultiSampleTypeList;
    if (pMultiSampleQualityList != NULL)  delete pMultiSampleQualityList;
    if (pDSMSConflictList != NULL)        delete pDSMSConflictList;
    if (pVertexProcessingTypeList != NULL) delete pVertexProcessingTypeList;
    if (pPresentIntervalList != NULL)     delete pPresentIntervalList;
}

//-----------------------------------------------------------------------------
// CD3DEnumeration constructor
//-----------------------------------------------------------------------------
...
CD3DEnumeration::~CD3DEnumeration()
{
    if (m_pAdapterInfoList != NULL)
    {
        for (UINT iai = 0; iai < m_pAdapterInfoList->Count(); iai++)
            delete static_cast<D3DAdapterInfo*>(m_pAdapterInfoList->GetPtr(iai));
...
HRESULT CD3DEnumeration::Enumerate()
{
    ...
    // Add allowed adapter formats
    D3DFORMAT fmt;
    if (FAILED(hr = m_pAllowedAdapterFormatList->Add(&(fmt = D3DFMT_X8R8G8B8))))    return hr;
    if (FAILED(hr = m_pAllowedAdapterFormatList->Add(&(fmt = D3DFMT_X1R5G5B5))))    return hr;
    if (FAILED(hr = m_pAllowedAdapterFormatList->Add(&(fmt = D3DFMT_R5G6B5))))      return hr;
    if (FAILED(hr = m_pAllowedAdapterFormatList->Add(&(fmt = D3DFMT_A2R10G10B10)))) return hr;
    ...
    for (UINT adapterOrdinal = 0; adapterOrdinal < numAdapters; adapterOrdinal++)
    {
        ...
        {
            D3DFORMAT allowedAdapterFormat = *static_cast<D3DFORMAT*>(m_pAllowedAdapterFormatList->GetPtr(iaaf));
...
HRESULT CD3DEnumeration::EnumerateDevices(D3DAdapterInfo* pAdapterInfo, CArrayList* pAdapterFormatList)
{
    ...
    for (UINT idt = 0; idt < devTypeArrayCount; idt++)
    {
        ...
        // If at least one devicecombo for this device is found, add the deviceInfo to the list
...
HRESULT CD3DEnumeration::EnumerateDeviceCombos(D3DDeviceInfo* pDeviceInfo, CArrayList* pAdapterFormatList)
{
    ...
    {
        D3DFORMAT adapterFormat = *static_cast<D3DFORMAT*>(pAdapterFormatList->GetPtr(iaf));
...
void CD3DEnumeration::BuildDepthStencilFormatList(D3DDeviceCombo* pDeviceCombo)
{
    ...
    for (UINT idsf = 0; idsf < depthStencilFormatArrayCount; idsf++)
    {
        ...
        if (DepthBits(depthStencilFmt) < AppMinDepthBits)     continue;
        if (StencilBits(depthStencilFmt) < AppMinStencilBits) continue;
...
void CD3DEnumeration::BuildDSMSConflictList(D3DDeviceCombo* pDeviceCombo)
{
    ...
    {
        D3DFORMAT dsFmt = *static_cast<D3DFORMAT*>(pDeviceCombo->pDepthStencilFormatList->GetPtr(ids));

        for (UINT ims = 0; ims < pDeviceCombo->pMultiSampleTypeList->Count(); ims++)
        {
            D3DMULTISAMPLE_TYPE msType = *static_cast<D3DMULTISAMPLE_TYPE*>(pDeviceCombo->pMultiSampleTypeList->GetPtr(ims));
