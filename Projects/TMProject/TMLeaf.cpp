...
int TMLeaf::InitObject()
{
    if (!m_pSkinMesh)
    {
        ...
        if (m_pSkinMesh)
        {
            ...
            LOG_WRITELOG("TEXO-LEAF: InitObject OK mesh=%p type=%d pos=(%.1f,%.1f)\r\n", 
                m_pSkinMesh, m_nSkinMeshType, m_vecPosition.x, m_vecPosition.y);
        }
        else
        {
            LOG_WRITELOG("TEXO-LEAF: InitObject FAILED - TMSkinMesh null\r\n");
        }
    }
    else
    {
        LOG_WRITELOG("TEXO-LEAF: InitObject skipped - mesh already exists\r\n");
    }
...
int TMLeaf::Render()
{
    // DEBUG: Log entry and early exits
    static int s_RenderCount = 0;
    if (s_RenderCount < 10)
    {
        LOG_WRITELOG("TEXO-LEAF-RENDER: Enter pos=(%.1f,%.1f) key=%u visible=%d skinmesh=%p\r\n",
            m_vecPosition.x, m_vecPosition.y, m_dwKey, m_bVisible, m_pSkinMesh);
        ++s_RenderCount;
    }

    if (g_bHideBackground == 1)
    {
        LOG_WRITELOG("TEXO-LEAF-RENDER: Exit - g_bHideBackground=1\r\n");
        return 0;
    }
    ...
    // DEBUG: Temporarily disable key check to test rendering
    // if (m_dwKey != -1 &&
    ...
    // {
    //     static int s_KeySkip = 0;
    //     if (s_KeySkip < 10)
    //     {
    //         LOG_WRITELOG("TEXO-LEAF-RENDER: Exit - key mismatch. key=%u vs [%u,%u,%u,%u]\r\n",
    //             m_dwKey, TreeNode::m_VisualKey1, TreeNode::m_VisualKey2, 
    //             TreeNode::m_VisualKey3, TreeNode::m_VisualKey4);
    //         ++s_KeySkip;
    //     }
    //     return 0;
    // }
    ...
    {
        static int s_RenderOK = 0;
        if (s_RenderOK < 10)
        {
            LOG_WRITELOG("TEXO-LEAF-RENDER: RENDERING! pos=(%.1f,%.1f)\r\n", m_vecPosition.x, m_vecPosition.y);
            ++s_RenderOK;
        }
        ...
        float s = (m_fScaleH + m_fScaleV) * 0.5f;
        m_pSkinMesh->Render(0.0f, s, 0.0f);
...
int TMLeaf::FrameMove(unsigned int dwServerTime)
{
    ...
    // DEBUG: Temporarily disable key check in FrameMove
