...
int D3DDevice::Initialize(HWND hWnd)
{
    ...
    // Don't save window style for borderless fullscreen - we want to keep our WS_POPUP
...
// ---------------------------------------------------------------------------
// GetVertexProcessingFlags
// ---------------------------------------------------------------------------

DWORD D3DDevice::GetVertexProcessingFlags(VertexProcessingType vpt)
{
    if (vpt == SOFTWARE_VP)
        return D3DCREATE_SOFTWARE_VERTEXPROCESSING;
    else if (vpt == MIXED_VP)
        return D3DCREATE_MIXED_VERTEXPROCESSING;
    else if (vpt == HARDWARE_VP)
        return D3DCREATE_HARDWARE_VERTEXPROCESSING;
    else if (vpt == PURE_HARDWARE_VP)
...
{
    DWORD dwBehavior = GetVertexProcessingFlags(vertexProcessingType);
    ...
    if (g_nAntialias)
    {
        for (int i = 1; i <= g_nAntialias; ++i)
        {
            ...
            if (m_pD3D->CheckDeviceMultiSampleType(0, D3DDEVTYPE::D3DDEVTYPE_HAL, m_d3dpp.BackBufferFormat,
                m_bWindowed, static_cast<D3DMULTISAMPLE_TYPE>(i + 1), &qualityLevel) >= 0 &&
                m_pD3D->CheckDeviceMultiSampleType(0, D3DDEVTYPE::D3DDEVTYPE_HAL, m_d3dpp.AutoDepthStencilFormat,
                    m_bWindowed, static_cast<D3DMULTISAMPLE_TYPE>(nTempLevel), &qualityLevel) >= 0)
            {
                m_nAntiAliasLevel = nTempLevel;
                m_d3dSettings.SetMultisampleType(static_cast<D3DMULTISAMPLE_TYPE>(nTempLevel));
...
char D3DDevice::FindBestWindowedMode(bool bRequireHAL, bool bRequireREF)
{
    ...
    {
        D3DAdapterInfo* pAdapterInfo = static_cast<D3DAdapterInfo*>(m_d3dEnumeration.m_pAdapterInfoList->GetPtr(iai));

        for (UINT idi = 0; idi < pAdapterInfo->pDeviceInfoList->Count(); idi++)
        {
            D3DDeviceInfo* pDeviceInfo = static_cast<D3DDeviceInfo*>(pAdapterInfo->pDeviceInfoList->GetPtr(idi));
            ...
            {
                D3DDeviceCombo* pDeviceCombo = static_cast<D3DDeviceCombo*>(pDeviceInfo->pDeviceComboList->GetPtr(idc));
...
EndWindowedDeviceComboSearch:
    ...
    if (m_d3dEnumeration.AppUsesDepthBuffer)
        m_d3dSettings.Windowed_DepthStencilBufferFormat = *static_cast<D3DFORMAT*>(pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0));

    m_d3dSettings.Windowed_MultisampleType = *static_cast<D3DMULTISAMPLE_TYPE*>(pBestDeviceCombo->pMultiSampleTypeList->GetPtr(0));
    m_d3dSettings.Windowed_MultisampleQuality = 0;
    m_d3dSettings.Windowed_VertexProcessingType = *static_cast<VertexProcessingType*>(pBestDeviceCombo->pVertexProcessingTypeList->GetPtr(0));
    m_d3dSettings.Windowed_PresentInterval = *static_cast<UINT*>(pBestDeviceCombo->pPresentIntervalList->GetPtr(0));
...
EndFullscreenDeviceComboSearch:
    ...
    {
        D3DDISPLAYMODE* pdm = static_cast<D3DDISPLAYMODE*>(pBestAdapterInfo->pDisplayModeList->GetPtr(idm));
    ...
    if (m_d3dEnumeration.AppUsesDepthBuffer)
        m_d3dSettings.Fullscreen_DepthStencilBufferFormat = *static_cast<D3DFORMAT*>(pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0));

    m_d3dSettings.Fullscreen_MultisampleType = *static_cast<D3DMULTISAMPLE_TYPE*>(pBestDeviceCombo->pMultiSampleTypeList->GetPtr(0));
    m_d3dSettings.Fullscreen_MultisampleQuality = 0;
    m_d3dSettings.Fullscreen_VertexProcessingType = *static_cast<VertexProcessingType*>(pBestDeviceCombo->pVertexProcessingTypeList->GetPtr(0));
...
// ---------------------------------------------------------------------------
// DetectGPUFromAdapter
// ---------------------------------------------------------------------------

void D3DDevice::DetectGPUFromAdapter(const D3DAdapterInfo* pAdapterInfo)
{
    char szDesc[512];
    strcpy_s(szDesc, pAdapterInfo->AdapterIdentifier.Description);
    char* vgaUP = _strupr(szDesc);
...
void D3DDevice::BuildDeviceStats(const D3DAdapterInfo* pAdapterInfo, const D3DDeviceInfo* pDeviceInfo, DWORD behaviorFlags)
{
...
HRESULT D3DDevice::Initialize3DEnvironment()
{
    ...
    DetectGPUFromAdapter(pAdapterInfo);
    ...
    DWORD behaviorFlags = GetVertexProcessingFlags(m_d3dSettings.GetVertexProcessingType());

    hr = m_pD3D->CreateDevice(m_d3dSettings.AdapterOrdinal(), pDeviceInfo->DevType,
        m_hWndFocus, behaviorFlags, &m_d3dpp, &m_pd3dDevice);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // Build and store device stats string
        BuildDeviceStats(pAdapterInfo, pDeviceInfo, behaviorFlags);
...
HRESULT D3DDevice::AdjustWindowForChange()
{
    if (m_bWindowed)
    {
        // Borderless fullscreen: apply WS_POPUP | WS_VISIBLE even in "windowed" mode
        if (m_bBorderlessFullscreen)
        {
    ...
    {
        // Borderless fullscreen: WS_POPUP | WS_VISIBLE (0x90000000)
        SetWindowLong(m_hWnd, GWL_STYLE, WS_POPUP | WS_VISIBLE);
        if (!m_hMenu)
        {
...
// ---------------------------------------------------------------------------
// Virtual overrides (base implementations)
// ---------------------------------------------------------------------------
...
void D3DDevice::CaptureScreen()
{
    ...
    sprintf(reinterpret_cast<char*>(szFileName), "ScreenShot\\Capture%04d.bmp", 0);

    do
    {
        handle = _open(reinterpret_cast<char*>(szFileName), _O_BINARY);
        if (handle < 0)
            break;

        _close(handle);
        sprintf(reinterpret_cast<char*>(szFileName), "ScreenShot\\Capture%04d.bmp", ++nCount);
    } while (nCount <= 9999);
