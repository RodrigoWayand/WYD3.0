...
static void InitSkyOverrideDefaults(TMGround* pGround)
{
	if (!pGround)
		return;
	pGround->m_bSkyOverride = 0;
	pGround->m_sSkyFlags = 0;
	pGround->m_fSkyFogStart = 0.0f;
	pGround->m_fSkyFogEnd = 0.0f;
	pGround->m_dwSkyClearColor = 0;
	pGround->m_nSkyState = -1;
	ZeroMemory(&pGround->m_SkyLight, sizeof(pGround->m_SkyLight));
	ZeroMemory(&pGround->m_SkyBackLight, sizeof(pGround->m_SkyBackLight));
}

static void InitRenderOverrideDefaults(TMGround* pGround)
{
	if (!pGround)
		return;
	pGround->m_bRenderOverride = 0;
	pGround->m_sRenderFlags = 0;
	pGround->m_fRenderClipFar = 0.0f;
	pGround->m_fRenderCullingRadius = 0.0f;
	pGround->m_fRenderObjectCulling = 0.0f;
}
...
static int TryLoadSkySidecar(TMGround* pGround, const char* trnPath)
{
	if (!pGround || !trnPath || !trnPath[0])
		return 0;

	InitSkyOverrideDefaults(pGround);

	char skyPath[128]{};
	strncpy_s(skyPath, trnPath, sizeof(skyPath) - 1);
	char* dot = strrchr(skyPath, '.');
	if (dot)
		strcpy_s(dot, sizeof(skyPath) - (dot - skyPath), ".sky");
	else
		strcat_s(skyPath, ".sky");

	int h = _open(skyPath, _O_BINARY);
	if (h == -1)
		return 0;
	...
	SkyHeader hdr{};
	int r = _read(h, &hdr, sizeof(hdr));
	if (r != sizeof(hdr) || hdr.magic[0] != 'S' || hdr.magic[1] != 'K' || hdr.magic[2] != 'Y' || hdr.magic[3] != 'O')
	{
		_close(h);
		return 0;
	}
	...
	// 0x0008: light
	// 0x0010: backLight
	pGround->m_bSkyOverride = 1;
	pGround->m_sSkyFlags = hdr.flags;

	if (hdr.flags & 0x0001)
	{
		float fogStart = 0.0f;
		float fogEnd = 0.0f;
		if (_read(h, &fogStart, sizeof(fogStart)) == sizeof(fogStart) && _read(h, &fogEnd, sizeof(fogEnd)) == sizeof(fogEnd))
		{
			fogStart = ClampFloat(fogStart, 0.0f, 500.0f);
			fogEnd = ClampFloat(fogEnd, 0.0f, 500.0f);
			if (fogEnd < fogStart + 5.0f)
				fogEnd = fogStart + 5.0f;
			pGround->m_fSkyFogStart = fogStart;
			pGround->m_fSkyFogEnd = fogEnd;
		}
	}
	...
	if (hdr.flags & 0x0002)
	{
		...
		if (_read(h, &clearColor, sizeof(clearColor)) == sizeof(clearColor))
			pGround->m_dwSkyClearColor = clearColor;
	}

	if (hdr.flags & 0x0004)
	{
		int skyState = -1;
		if (_read(h, &skyState, sizeof(skyState)) == sizeof(skyState))
			pGround->m_nSkyState = skyState;
	}

	if (hdr.flags & 0x0008)
	{
		D3DCOLORVALUE v{};
		if (_read(h, &v, sizeof(v)) == sizeof(v))
			pGround->m_SkyLight = v;
	}
...
static int TryLoadRenderSidecar(TMGround* pGround, const char* trnPath)
{
	if (!pGround || !trnPath || !trnPath[0])
		return 0;

	InitRenderOverrideDefaults(pGround);

	char renderPath[128]{};
	strncpy_s(renderPath, trnPath, sizeof(renderPath) - 1);
	char* dot = strrchr(renderPath, '.');
	if (dot)
		strcpy_s(dot, sizeof(renderPath) - (dot - renderPath), ".render");
	else
		strcat_s(renderPath, ".render");

	int h = _open(renderPath, _O_BINARY);
	if (h == -1)
	{
		LOG_WRITELOG("Render sidecar not found: %s\r\n", renderPath);
		return 0;
	}
	...
	RenderHeader hdr{};
	int r = _read(h, &hdr, sizeof(hdr));
	if (r != sizeof(hdr) || hdr.magic[0] != 'R' || hdr.magic[1] != 'N' || hdr.magic[2] != 'D' || hdr.magic[3] != 'R')
	{
		LOG_WRITELOG("Render sidecar invalid header: %s\r\n", renderPath);
		_close(h);
		return 0;
	}
	...
	if (hdr.version != 1)
	{
		LOG_WRITELOG("Render sidecar bad version: %s ver=%d\r\n", renderPath, hdr.version);
		_close(h);
		return 0;
	}
	...
	// flags (bitmask)
	// 0x0001: clipFar
	// 0x0002: cullingRadius (reserved)
	// 0x0004: objectCulling (reserved)
	pGround->m_bRenderOverride = 1;
	pGround->m_sRenderFlags = hdr.flags;

	LOG_WRITELOG("Render sidecar loaded: %s flags=0x%04X\r\n", renderPath, hdr.flags);

	if (hdr.flags & 0x0001)
	{
		float clipFar = 0.0f;
		if (_read(h, &clipFar, sizeof(clipFar)) == sizeof(clipFar))
		{
			clipFar = ClampFloat(clipFar, 70.0f, 1000.0f);
			pGround->m_fRenderClipFar = clipFar;
			LOG_WRITELOG("  clipFar=%.1f\r\n", clipFar);
		}
	}
	...
	if (hdr.flags & 0x0002)
	{
		...
		if (_read(h, &cullRadius, sizeof(cullRadius)) == sizeof(cullRadius))
		{
			cullRadius = ClampFloat(cullRadius, 0.0f, 5000.0f);
			pGround->m_fRenderCullingRadius = cullRadius;
			LOG_WRITELOG("  cullRadius=%.1f\r\n", cullRadius);
		}
	}
	...
	{
		float objCull = 0.0f;
		if (_read(h, &objCull, sizeof(objCull)) == sizeof(objCull))
		{
			objCull = ClampFloat(objCull, 0.0f, 5000.0f);
			pGround->m_fRenderObjectCulling = objCull;
			LOG_WRITELOG("  objCull=%.1f\r\n", objCull);
		}
	}
...
static void ApplySkyOverrideIfAny(TMGround* pGround)
{
	if (!pGround)
		return;
	if (!pGround->m_bSkyOverride)
		return;

	if ((pGround->m_sSkyFlags & 0x0004) && g_pCurrentScene && g_pCurrentScene->m_pSky)
	{
		if (pGround->m_nSkyState >= 0)
			g_pCurrentScene->m_pSky->SetWeatherState(pGround->m_nSkyState);
	}
	if (pGround->m_sSkyFlags & 0x0001)
	{
		g_pDevice->m_fFogStart = pGround->m_fSkyFogStart;
		g_pDevice->m_fFogEnd = pGround->m_fSkyFogEnd;
	}
	if (pGround->m_sSkyFlags & 0x0002)
	{
		g_pDevice->m_dwClearColor = pGround->m_dwSkyClearColor;
	}
	if ((pGround->m_sSkyFlags & 0x0008))
	{
		g_pDevice->m_colorLight = pGround->m_SkyLight;
	}
	if ((pGround->m_sSkyFlags & 0x0010))
	{
		g_pDevice->m_colorBackLight = pGround->m_SkyBackLight;
	}
}

static void ApplyRenderOverrideIfAny(TMGround* pGround)
{
	if (!pGround)
		return;
	...
	if (pGround->m_sRenderFlags & 0x0001)
	{
		if (pGround->m_fRenderClipFar > 0.0f)
			g_ClipFar = pGround->m_fRenderClipFar;
	}
	if (pGround->m_sRenderFlags & 0x0002)
	{
		if (pGround->m_fRenderCullingRadius > 0.0f)
		{
			float desiredClipFar = pGround->m_fRenderCullingRadius * 6.0f;
			if (desiredClipFar < 200.0f)
				desiredClipFar = 200.0f;
			if (desiredClipFar > g_ClipFar)
				g_ClipFar = desiredClipFar;
		}
	}
}
...
int TMScene::OnKeyDownEvent(unsigned int iKeyCode)
{
	...
	if (iKeyCode == VK_F10)
	{
		auto pCam = (g_pObjectManager ? g_pObjectManager->m_pCamera : nullptr);
		if (pCam)
			pCam->m_bLockCamera = (pCam->m_bLockCamera == 0);
		return 1;
	}
...
int TMScene::FrameMove(unsigned int dwServerTime)
{
	...
	{
		// POC1: keep neighbor terrain (TRN) loaded around the current ground with a small per-frame budget.
		// This does NOT load objects/props; it only ensures adjacent grounds exist for a continuous-world feel.
		// Enable/disable via compile-time flag.
		#if defined(STREAM_POC1_TRN_3X3)
		{
			static TMVector2 s_lastPos{};
			static bool s_hasLastPos = false;
			TMVector2 curPos(pFocusedObject->m_vecPosition.x, pFocusedObject->m_vecPosition.y);
			TMVector2 delta(0.0f, 0.0f);
			if (s_hasLastPos)
			{
				delta.x = curPos.x - s_lastPos.x;
				delta.y = curPos.y - s_lastPos.y;
			}
			s_lastPos = curPos;
			s_hasLastPos = true;

			// Budget: at most 1 TRN attach per frame.
			int budget = 1;

			auto tryAttach = [&](EDirection dir, TMGround* neighbor) -> bool
			{
				if (budget <= 0)
					return false;
				if (neighbor)
					return false;
				if (GroundNewAttach(dir, false) == 1)
				{
					--budget;
					return true;
				}
			...
			// Prioritize attaches based on movement direction.
			if (fabsf(delta.x) >= fabsf(delta.y))
			{
				if (delta.x > 0.0f)
					tryAttach(EDirection::EDIR_RIGHT, m_pGround->m_pRightGround);
				else if (delta.x < 0.0f)
					tryAttach(EDirection::EDIR_LEFT, m_pGround->m_pLeftGround);
			}
			else
			{
				if (delta.y > 0.0f)
					tryAttach(EDirection::EDIR_DOWN, m_pGround->m_pDownGround);
				else if (delta.y < 0.0f)
					tryAttach(EDirection::EDIR_UP, m_pGround->m_pUpGround);
			}

			// Fill remaining budget with any missing neighbor (stable priority).
			if (budget > 0)
				tryAttach(EDirection::EDIR_LEFT, m_pGround->m_pLeftGround);
			if (budget > 0)
				tryAttach(EDirection::EDIR_RIGHT, m_pGround->m_pRightGround);
		...
		// B1 streaming: prefetch neighbor ground early (TRN only), then load neighbor objects closer to the border.
		if (dX >= 0.0f && (float)nPrefetchShold > dX)
		{
			if (!m_pGround->m_pLeftGround)
				GroundNewAttach(EDirection::EDIR_LEFT, false);
			else if ((float)nObjectShold > dX)
				GroundNewAttach(EDirection::EDIR_LEFT, true);
		}
		else if (dX > (float)(128 - nPrefetchShold) && dX < 128.0f)
		{
			if (!m_pGround->m_pRightGround)
				GroundNewAttach(EDirection::EDIR_RIGHT, false);
			else if (dX > (float)(128 - nObjectShold))
				GroundNewAttach(EDirection::EDIR_RIGHT, true);
		}

		if (dY >= 0.0f && (float)nPrefetchShold > dY)
		{
			if (!m_pGround->m_pUpGround)
				GroundNewAttach(EDirection::EDIR_UP, false);
			else if ((float)nObjectShold > dY)
				GroundNewAttach(EDirection::EDIR_UP, true);
		}
		else if (dY > (float)(128 - nPrefetchShold) && (float)128 > dY)
		{
			if (!m_pGround->m_pDownGround)
				GroundNewAttach(EDirection::EDIR_DOWN, false);
			else if (dY > (float)(128 - nObjectShold))
				GroundNewAttach(EDirection::EDIR_DOWN, true);
		}
		...
		else if (dX < 0.0f && dX > (float)-nShold)
		{
			if (m_pGround->m_pLeftGround)
			{
				...
				ApplySkyOverrideIfAny(m_pGround);
				ApplyRenderOverrideIfAny(m_pGround);
				if (!m_pObjectContainerList[m_nCurrentGroundIndex])
				{
					char szDataPath[128]{};
					sprintf(szDataPath, "env\\Field%02d%02d.dat", m_pGround->m_vecOffsetIndex.x, m_pGround->m_vecOffsetIndex.y);
					m_pObjectContainerList[m_nCurrentGroundIndex] = new TMObjectContainer(m_pGround);
					if (!m_pObjectContainerList[m_nCurrentGroundIndex]->Load(szDataPath))
					{
					...
					m_pGroundObjectContainer->AddChild(m_pObjectContainerList[m_nCurrentGroundIndex]);
				}

				// Recompute collision/height data after LEFT swap
				g_HeightPosX = (int)m_pGround->m_vecOffset.x;
				g_HeightPosY = (int)m_pGround->m_vecOffset.y;
				for (int i = 0; i < 128; ++i)
					memcpy(m_HeightMapData[i], m_pGround->m_pMaskData[i], 128);
				BASE_ApplyAttribute((char*)m_HeightMapData, 256);
				memcpy(m_GateMapData, m_HeightMapData, sizeof(m_HeightMapData));

				// Force camera to recalculate VisualKeys for the new ground
				if (g_pObjectManager && g_pObjectManager->m_pCamera)
					g_pObjectManager->m_pCamera->GetCameraPos();
			}
		}
		...
		else if (dX >= 128.0f && (float)(nShold + 128) > dX && m_pGround->m_pRightGround)
		{
			...
			// Recompute collision/height data after RIGHT swap
			g_HeightPosX = (int)m_pGround->m_vecOffset.x;
			g_HeightPosY = (int)m_pGround->m_vecOffset.y;
			for (int i = 0; i < 128; ++i)
				memcpy(m_HeightMapData[i], m_pGround->m_pMaskData[i], 128);
		...
		else if (dY < 0.0f && dY > (float)-nShold)
		{
			if (m_pGround->m_pUpGround)
			{
				...
				// Recompute collision/height data after UP swap
				g_HeightPosX = (int)m_pGround->m_vecOffset.x;
				g_HeightPosY = (int)m_pGround->m_vecOffset.y;
				for (int i = 0; i < 128; ++i)
					memcpy(m_HeightMapData[i], m_pGround->m_pMaskData[i], 128);
		...
		else if (dY >= (float)128 && (float)(nShold + 128) > dY && m_pGround->m_pDownGround)
		{
			...
			// Recompute collision/height data after DOWN swap
			g_HeightPosX = (int)m_pGround->m_vecOffset.x;
			g_HeightPosY = (int)m_pGround->m_vecOffset.y;
			for (int i = 0; i < 128; ++i)
				memcpy(m_HeightMapData[i], m_pGround->m_pMaskData[i], 128);
...
{
	return GroundNewAttach(eDir, true);
}

int TMScene::GroundNewAttach(EDirection eDir, bool bLoadObjects)
{
	...
	// If neighbor ground already exists, only load objects if requested and not loaded yet.
	TMGround* pExistingNeighbor = nullptr;
	if (eDir == EDirection::EDIR_LEFT)
		pExistingNeighbor = m_pGround->m_pLeftGround;
	else if (eDir == EDirection::EDIR_RIGHT)
		pExistingNeighbor = m_pGround->m_pRightGround;
	else if (eDir == EDirection::EDIR_UP)
		pExistingNeighbor = m_pGround->m_pUpGround;
	else if (eDir == EDirection::EDIR_DOWN)
		pExistingNeighbor = m_pGround->m_pDownGround;

	if (pExistingNeighbor)
	{
		if (!bLoadObjects)
			return 1;
		...
		for (int i = 0; i < 2; ++i)
		{
			if (m_pGroundList[i] == pExistingNeighbor)
			{
				idx = i;
				break;
			}
		}
		if (idx >= 0 && !m_pObjectContainerList[idx])
		{
			m_pObjectContainerList[idx] = new TMObjectContainer(m_pGroundList[idx]);
			if (!m_pObjectContainerList[idx]->Load(fileNameDat))
			{
				SAFE_DELETE(m_pObjectContainerList[idx]);
				LOG_WRITELOG("DataFile Not Found : %s\r\n", fileNameDat);
				if (!m_bCriticalError)
			...
			m_pGroundObjectContainer->AddChild(m_pObjectContainerList[idx]);

			if (m_pObjectContainerList[m_nCurrentGroundIndex])
			{
				if (eDir == EDirection::EDIR_DOWN)
					m_pObjectContainerList[m_nCurrentGroundIndex]->SetPrevNode(m_pObjectContainerList[idx]);
				else
					m_pObjectContainerList[m_nCurrentGroundIndex]->SetNextNode(m_pObjectContainerList[idx]);
			}
	...
	TryLoadSkySidecar(pGround, fileNameTrn);
	TryLoadRenderSidecar(pGround, fileNameTrn);
	...
	if (bLoadObjects)
	{
		...
		m_pGroundObjectContainer->AddChild(m_pObjectContainerList[gId]);
	...
	// DEBUG: Log sample values from m_HeightMapData for Field1615
	if (pGround->m_vecOffsetIndex.x == 16 && pGround->m_vecOffsetIndex.y == 15)
	{
		LOG_WRITELOG("TEXO-HEIGHT1615-AFTER: idx=[50,100]=%d idx=[60,100]=%d idx=[40,100]=%d\r\n",
			(signed char)m_HeightMapData[100][50], 
			(signed char)m_HeightMapData[100][60],
			(signed char)m_HeightMapData[100][40]);
	}
...
int TMScene::GroundGetMask(TMVector2 vecPosition)
{
	...
	// DEBUG: Log collision check for Field1615
	static int s_DbgCount = 0;
	if (m_pGround && m_pGround->m_vecOffsetIndex.x == 16 && m_pGround->m_vecOffsetIndex.y == 15 && s_DbgCount < 50)
	{
		TMGround* pMaskGround = m_pGround;
		if (pMaskGround && !(vecPosition.x >= pMaskGround->m_vecOffset.x && vecPosition.x < (pMaskGround->m_vecOffset.x + 128.0f) &&
			vecPosition.y >= pMaskGround->m_vecOffset.y && vecPosition.y < (pMaskGround->m_vecOffset.y + 128.0f)))
		{
			if (m_pGround->m_pLeftGround
				&& vecPosition.x >= m_pGround->m_pLeftGround->m_vecOffset.x
				&& vecPosition.x < (m_pGround->m_pLeftGround->m_vecOffset.x + 128.0f)
				&& vecPosition.y >= m_pGround->m_pLeftGround->m_vecOffset.y
				&& vecPosition.y < (m_pGround->m_pLeftGround->m_vecOffset.y + 128.0f))
				pMaskGround = m_pGround->m_pLeftGround;
			else if (m_pGround->m_pRightGround
				&& vecPosition.x >= m_pGround->m_pRightGround->m_vecOffset.x
				&& vecPosition.x < (m_pGround->m_pRightGround->m_vecOffset.x + 128.0f)
				&& vecPosition.y >= m_pGround->m_pRightGround->m_vecOffset.y
				&& vecPosition.y < (m_pGround->m_pRightGround->m_vecOffset.y + 128.0f))
				pMaskGround = m_pGround->m_pRightGround;
			else if (m_pGround->m_pUpGround
				&& vecPosition.x >= m_pGround->m_pUpGround->m_vecOffset.x
				&& vecPosition.x < (m_pGround->m_pUpGround->m_vecOffset.x + 128.0f)
				&& vecPosition.y >= m_pGround->m_pUpGround->m_vecOffset.y
				&& vecPosition.y < (m_pGround->m_pUpGround->m_vecOffset.y + 128.0f))
				pMaskGround = m_pGround->m_pUpGround;
			else if (m_pGround->m_pDownGround
				&& vecPosition.x >= m_pGround->m_pDownGround->m_vecOffset.x
				&& vecPosition.x < (m_pGround->m_pDownGround->m_vecOffset.x + 128.0f)
				&& vecPosition.y >= m_pGround->m_pDownGround->m_vecOffset.y
				&& vecPosition.y < (m_pGround->m_pDownGround->m_vecOffset.y + 128.0f))
				pMaskGround = m_pGround->m_pDownGround;
		}

		int maskX = -1;
		int maskY = -1;
		int groundMaskVal = -999;
		if (pMaskGround)
		{
			maskX = (int)(vecPosition.x - pMaskGround->m_vecOffset.x);
			maskY = (int)(vecPosition.y - pMaskGround->m_vecOffset.y);
			if (maskX >= 0 && maskY >= 0 && maskX < 128 && maskY < 128)
				groundMaskVal = (signed char)pMaskGround->m_pMaskData[maskY][maskX];
		}

		int globalVal = (nXIndex >= 0 && nXIndex < 256 && nYIndex >= 0 && nYIndex < 256) ? (signed char)m_HeightMapData[nYIndex][nXIndex] : -999;
		LOG_WRITELOG("COLISION: pos=(%.1f,%.1f) idx=(%d,%d) g_HeightPos=(%d,%d) global=%d ground=(%d,%d) m=(%d,%d) gmask=%d\r\n",
			vecPosition.x, vecPosition.y,
			nXIndex, nYIndex,
			g_HeightPosX, g_HeightPosY,
			globalVal,
			pMaskGround ? pMaskGround->m_vecOffsetIndex.x : -1,
			pMaskGround ? pMaskGround->m_vecOffsetIndex.y : -1,
			maskX, maskY,
			groundMaskVal);
		++s_DbgCount;
	}
...
void TMScene::Warp2(int nZoneX, int nZoneY)
{
		 ...
		 nZoneY != m_pGround->m_vecOffsetIndex.y && nZoneY != pNeighbor->m_vecOffsetIndex.y))
	{
		...
		TryLoadSkySidecar(pGround, szMapPath);
		TryLoadRenderSidecar(pGround, szMapPath);
		...
		// Force camera to recalculate VisualKeys for the new position/ground
		if (g_pObjectManager && g_pObjectManager->m_pCamera)
			g_pObjectManager->m_pCamera->GetCameraPos();
	}
	else if (pNeighbor && nZoneX == pNeighbor->m_vecOffsetIndex.x && nZoneY == pNeighbor->m_vecOffsetIndex.y)
	{
		...
		// If this ground was prefetched with TRN only, its object container may not exist yet.
		// Ensure DAT/EXT are loaded for the current ground so props appear and we avoid unstable states.
		if (!m_pObjectContainerList[m_nCurrentGroundIndex])
		{
			char szDataPath[128]{};
			sprintf(szDataPath, "env\\Field%02d%02d.dat", nZoneX, nZoneY);
...
int TMScene::ReadTXT(const char* szFileName, int nStartLine, char* Text)
{
	FILE* fp{};
