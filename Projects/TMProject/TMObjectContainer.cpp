...
{
	// DEBUG: Contadores de objetos por tipo
	int dbg_LeafCount = 0;
	int dbg_TreeCount = 0;
	int dbg_EffectMeshCount = 0;
	int dbg_EffectBillCount = 0;
	int dbg_OtherCount = 0;
	int dbg_TotalObjects = 0;

	TMVector2 vecPosition;	
	float m_fOffsetX = 0.0f;
	float m_fOffsetY = 0.0f;
	int pos = 0;
	unsigned char overrideType[MAX_OBJECT_LIST];
	int overrideIndex[MAX_OBJECT_LIST];
	float overrideScaleH[MAX_OBJECT_LIST];
	float overrideScaleV[MAX_OBJECT_LIST];
	unsigned int overrideHideMask[MAX_OBJECT_LIST];
	for (int i = 0; i < MAX_OBJECT_LIST; ++i)
	{
		overrideType[i] = 0;
		overrideIndex[i] = -1;
		overrideScaleH[i] = 1.0f;
		overrideScaleV[i] = 1.0f;
		overrideHideMask[i] = 0;
	}
	...
	strncpy_s(extFileName, szFileName, sizeof(extFileName) - 1);
	char* dot = strrchr(extFileName, '.');
	if (dot)
		strcpy_s(dot, sizeof(extFileName) - (dot - extFileName), ".ext");
	else
		strcat_s(extFileName, ".ext");

	LOG_WRITELOG("TEXO: Load dat=%s ext=%s\r\n", szFileName, extFileName);
	...
	unsigned int texoVersion = 0;
	unsigned int texoLoaded = 0;
	unsigned int texoScaled = 0;

	int extHandle = _open(extFileName, _O_BINARY);
	if (extHandle != -1)
	{
		stExtHeader hdr{};
		if (_read(extHandle, &hdr, sizeof(hdr)) == sizeof(hdr))
		{
			if (hdr.magic[0] == 'T' && hdr.magic[1] == 'E' && hdr.magic[2] == 'X' && hdr.magic[3] == 'O')
			{
				texoVersion = hdr.version;
				if (hdr.version == 1)
				{
					for (unsigned short i = 0; i < hdr.count; ++i)
					{
						stExtEntry ent{};
						if (_read(extHandle, &ent, sizeof(ent)) != sizeof(ent))
							break;
						if (ent.objectIndex < MAX_OBJECT_LIST)
						{
							++texoLoaded;
							overrideType[ent.objectIndex] = ent.type;
							overrideIndex[ent.objectIndex] = ent.textureIndex;
						}
					}
				}
				else if (hdr.version == 2)
				{
					for (unsigned short i = 0; i < hdr.count; ++i)
					{
						...
						if (ent.objectIndex < MAX_OBJECT_LIST)
						{
							...
							overrideType[ent.objectIndex] = ent.type;
							overrideIndex[ent.objectIndex] = ent.textureIndex;
							overrideScaleH[ent.objectIndex] = ent.scaleH > 0.0f ? ent.scaleH : 1.0f;
							overrideScaleV[ent.objectIndex] = ent.scaleV > 0.0f ? ent.scaleV : 1.0f;
							if (fabsf(overrideScaleH[ent.objectIndex] - 1.0f) > 0.0001f || fabsf(overrideScaleV[ent.objectIndex] - 1.0f) > 0.0001f)
								++texoScaled;
						}
					}
				}
				else if (hdr.version == 3)
				{
					for (unsigned short i = 0; i < hdr.count; ++i)
					{
						...
						if (ent.objectIndex < MAX_OBJECT_LIST)
						{
							...
							overrideScaleH[ent.objectIndex] = ent.scaleH > 0.0f ? ent.scaleH : 1.0f;
							overrideScaleV[ent.objectIndex] = ent.scaleV > 0.0f ? ent.scaleV : 1.0f;
							overrideHideMask[ent.objectIndex] = ent.hideMask;
							if (fabsf(overrideScaleH[ent.objectIndex] - 1.0f) > 0.0001f || fabsf(overrideScaleV[ent.objectIndex] - 1.0f) > 0.0001f)
								++texoScaled;
						}
					}
				...
				{
					LOG_WRITELOG("TEXO: versao desconhecida %u em %s\r\n", (unsigned int)hdr.version, extFileName);
				}
			}
			else
			{
				LOG_WRITELOG("TEXO: header invalido em %s\r\n", extFileName);
			}
		}
		_close(extHandle);
		if (texoVersion != 0)
			LOG_WRITELOG("TEXO: carregado v%u count=%u (scaled=%u) file=%s\r\n", texoVersion, texoLoaded, texoScaled, extFileName);
	}
	else
	{
		LOG_WRITELOG("TEXO: .ext nao encontrado (ou path truncado): %s\r\n", extFileName);
	}
	...
	{
		// Segurança: evitar ler item incompleto (arquivo truncado/corrompido)
		if (pos + sz1 > sz)
			break;
		...
		if (dwObjType == 2)
		{
			...
			if (pGround != nullptr)
			{
				// Segurança: evitar overflow do array m_pSeaList[10]
				if (pGround->m_nSeaIndex >= 0 && pGround->m_nSeaIndex < (int)(sizeof(pGround->m_pSeaList) / sizeof(pGround->m_pSeaList[0])))
				{
		...
		else if (dwObjType >= 501 && dwObjType < 506)
		{
			...
			// Segurança: validar leitura extra do item
			if (pos + sz2 > sz)
				break;
			pos += sz2;
			switch (dwObjType)
		...
		// Segurança: se a alocação falhar, evitar deref
		if (m_pObjectList[m_nObjectIndex] == nullptr)
			continue;
		...
		m_pObjectList[m_nObjectIndex]->m_nTexOverrideType = (int)overrideType[m_nObjectIndex];
		m_pObjectList[m_nObjectIndex]->m_nTexOverrideIndex = overrideIndex[m_nObjectIndex];
		m_pObjectList[m_nObjectIndex]->m_fScaleH = overrideScaleH[m_nObjectIndex];
		m_pObjectList[m_nObjectIndex]->m_fScaleV = overrideScaleV[m_nObjectIndex];
		m_pObjectList[m_nObjectIndex]->m_dwHidePartsMask = overrideHideMask[m_nObjectIndex];
		if (fabsf(m_pObjectList[m_nObjectIndex]->m_fScaleH - 1.0f) > 0.0001f || fabsf(m_pObjectList[m_nObjectIndex]->m_fScaleV - 1.0f) > 0.0001f)
		{
			LOG_WRITELOG("TEXO: apply idx=%d objType=%u scaleH=%.3f scaleV=%.3f pos=(%.1f,%.1f,%.1f)\r\n",
				m_nObjectIndex,
				(unsigned int)m_pObjectList[m_nObjectIndex]->m_dwObjType,
				m_pObjectList[m_nObjectIndex]->m_fScaleH,
				m_pObjectList[m_nObjectIndex]->m_fScaleV,
				m_fOffsetX + vecPosition.x,
				fHeight,
				m_fOffsetY + vecPosition.y);
		}
	...
	// DEBUG: Log object counts for this container
	if (m_pGround && strstr(szFileName, "1615"))
	{
		LOG_WRITELOG("TEXO-DEBUG: Field1615 ObjectCounts - Leaf=%d Tree=%d TotalInList=%d\r\n",
			dbg_LeafCount, dbg_TreeCount, m_nObjectIndex);
	}
