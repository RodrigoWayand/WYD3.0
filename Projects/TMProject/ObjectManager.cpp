...
{
	// [CORREÇÃO] Validação de cena e integridade do pacote para MSG_SendItem
	if (g_pCurrentScene != nullptr && g_pCurrentScene->m_eSceneType == ESCENE_TYPE::ESCENE_SELCHAR && buf != nullptr)
	{
		MSG_STANDARD* pStd = (MSG_STANDARD*)buf;
		if (pStd->Type == MSG_SendItem_Opcode)
		{
			MSG_SendItem* pMsg = (MSG_SendItem*)buf;
			if (pMsg->DestType == 2)
			{
				// [CORREÇÃO] Proteção contra índice fora dos limites
				if (pMsg->DestPos >= 0 && pMsg->DestPos < 128)
...
TMScene* ObjectManager::GetNodeByID(unsigned int dwID)
{
	...
	// [CORREÇÃO] Proteção contra cena nula
	if (g_pCurrentScene == nullptr)
		return nullptr;
