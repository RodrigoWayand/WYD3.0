...
void TMMesh::SetTexture(int nIndex)
{
	...
	if (g_TexOverride.active)
	{
		DWORD showTime = m_dwShowTime <= 10000 ? 10000u : m_dwShowTime;
		if (g_TexOverride.type == 1)
			texture = g_pTextureManager->GetEnvTexture(g_TexOverride.index, showTime);
		else if (g_TexOverride.type == 2)
			texture = g_pTextureManager->GetModelTexture(g_TexOverride.index, showTime);
		else if (g_TexOverride.type == 3)
			texture = g_pTextureManager->GetEffectTexture(g_TexOverride.index, showTime);
		else if (m_bEffect == 1)
...
HRESULT TMMesh::RenderDraw(int nTexOffset)
{
	...
	{
		if (g_MeshPartHideOverride.active && i < 32 && ((g_MeshPartHideOverride.hideMask >> (unsigned int)i) & 1u))
			continue;
