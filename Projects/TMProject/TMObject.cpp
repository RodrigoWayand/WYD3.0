...
int TMObject::Render()
{
	if (g_bHideBackground == 1)
		return 0;
	// Segurança: verificar se a cena atual existe antes de acessar
	if (!g_pCurrentScene)
		return 0;
	if (!isVisualKey())
	{
	...
	if (g_pDevice->m_bVoodoo == 1)
	{
		...
		int prevActive = g_TexOverride.active;
		int prevType = g_TexOverride.type;
		int prevIndex = g_TexOverride.index;
		if (m_nTexOverrideType > 0 && m_nTexOverrideIndex >= 0)
		{
			g_TexOverride.active = 1;
			g_TexOverride.type = m_nTexOverrideType;
			g_TexOverride.index = m_nTexOverrideIndex;
		}

		int prevHideActive = g_MeshPartHideOverride.active;
		unsigned int prevHideMask = g_MeshPartHideOverride.hideMask;
		if (m_dwHidePartsMask)
		{
			g_MeshPartHideOverride.active = 1;
			g_MeshPartHideOverride.hideMask = m_dwHidePartsMask;
		}

		float prevScaleH = pMesh->m_fScaleH;
		float prevScaleV = pMesh->m_fScaleV;
		static int sLoggedRender201Voodoo = 0;
		if (!sLoggedRender201Voodoo && m_dwObjType == 201 && (fabsf(m_fScaleH - 1.0f) > 0.0001f || fabsf(m_fScaleV - 1.0f) > 0.0001f))
		{
			sLoggedRender201Voodoo = 1;
			LOG_WRITELOG("TEXO: render201(voodoo) objScaleH=%.3f objScaleV=%.3f meshPrevH=%.3f meshPrevV=%.3f pos=(%.1f,%.1f,%.1f)\r\n",
				m_fScaleH,
				m_fScaleV,
				prevScaleH,
				prevScaleV,
		...
		pMesh->m_fScaleH = prevScaleH * m_fScaleH;
		pMesh->m_fScaleV = prevScaleV * m_fScaleV;
		pMesh->Render(m_vecPosition.x, m_fHeight, m_vecPosition.y, m_fAngle, 0, 0, 0, 0);
		pMesh->m_fScaleH = prevScaleH;
		pMesh->m_fScaleV = prevScaleV;
		g_TexOverride.active = prevActive;
		g_TexOverride.type = prevType;
		g_TexOverride.index = prevIndex;
		g_MeshPartHideOverride.active = prevHideActive;
		g_MeshPartHideOverride.hideMask = prevHideMask;
		g_pDevice->SetTextureStageState(1u, D3DTSS_COLOROP, 1u);
	...
	float prevScaleH = pMesh->m_fScaleH;
	float prevScaleV = pMesh->m_fScaleV;
	static int sLoggedRender201 = 0;
	if (!sLoggedRender201 && m_dwObjType == 201 && (fabsf(m_fScaleH - 1.0f) > 0.0001f || fabsf(m_fScaleV - 1.0f) > 0.0001f))
	{
		sLoggedRender201 = 1;
		LOG_WRITELOG("TEXO: render201 objScaleH=%.3f objScaleV=%.3f meshPrevH=%.3f meshPrevV=%.3f pos=(%.1f,%.1f,%.1f)\r\n",
			m_fScaleH,
			m_fScaleV,
			prevScaleH,
			prevScaleV,
...
int TMObject::IsVisible()
{
	...
	float fDist = D3DXVec3Length(&Vec);

	// Option A: when render override is active (0x0002), keep map props visible far away,
	// but limit dynamic objects (mobs/players/effects) via objectCulling (0x0004) to avoid
	// rendering entities over unloaded terrain (blue void).
	if (m_dwKey == (unsigned int)-1 && g_pCurrentScene && g_pCurrentScene->m_pGround)
	{
		auto pGround = g_pCurrentScene->m_pGround;
		if (pGround->m_bRenderOverride && (pGround->m_sRenderFlags & 0x0002) && (pGround->m_sRenderFlags & 0x0004) && pGround->m_fRenderObjectCulling > 0.0f)
		{
			if (fDist > pGround->m_fRenderObjectCulling + pMesh->m_fRadius)
			{
				m_bVisible = 0;
				return 0;
			}
...
int TMObject::isVisualKey()
{
	...
	{
		auto pGround = g_pCurrentScene->m_pGround;
		// Only bypass VisualKey culling for map/static objects that are keyed into the ground grid.
		// Dynamic objects (mobs/players) may not have a valid key and would otherwise render in the void.
		if (m_dwKey != (unsigned int)-1 && pGround->m_bRenderOverride)
		{
			...
			{
				float derived = pGround->m_fRenderClipFar / 6.0f;
				if (derived < 0.0f)
					derived = 0.0f;
				if (derived > 5000.0f)
					derived = 5000.0f;
...
int TMObject::isCamPos()
{
	...
	D3DXMATRIX mat;
	D3DXMATRIX mat2;
	D3DXMatrixScaling(&matScale, pMesh->m_fScaleH * m_fScaleH, pMesh->m_fScaleV * m_fScaleV, pMesh->m_fScaleH * m_fScaleH);
	D3DXMatrixRotationYawPitchRoll(&mat, m_fAngle, -D3DXToRadian(90), 0);
	D3DXMatrixMultiply(&mat, &g_pDevice->m_matWorld, &mat);
	D3DXMatrixMultiply(&mat, &mat, &matScale);
...
void TMObject::ResetObject()
{
	...
	m_fHeight = 0.0;
	m_nTexOverrideType = 0;
	m_nTexOverrideIndex = -1;
	m_dwHidePartsMask = 0;
	m_nSkinMeshType = 0;
}
