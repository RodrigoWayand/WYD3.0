...
#pragma region Novas Janelas
	...
	for (int i = 0; i < 20; i++) {
		memset(&m_CompNames[i], 0, 16);
		ReadTXT((char*)"UI\\NpcNames.txt", i, m_CompNames[i]);
	}
	...
	auto Guilda = (SPanel*)m_pControlContainer->FindControl(57572000);
	Guilda->SetVisible(0);

	auto JanelaGrupo = (SPanel*)m_pControlContainer->FindControl(3741000);
	JanelaGrupo->SetVisible(0);
...
// Novas Janelas - button control
	...
	int GrupoBtn = 0;
	if (SendHandleID >= 3741201 && SendHandleID <= 3741207) {
		GrupoBtn = SendHandleID - 3741201;
		SendHandleID = SELECIONARGRUPO;
	}
...
#pragma region Janela de Grupo
	case 1762199: {
		auto JanelaGrupo = (SPanel*)m_pControlContainer->FindControl(3741000);

		if (JanelaGrupo->IsVisible() == 0) {
			JanelaGrupo->SetVisible(1);
			break;
		}
		else {
			JanelaGrupo->SetVisible(0);
	...
	case 3741308: {
		g_pClientInfo->Info.GrupoSelecionado = -1;

		for (int i = 0; i < 7; i++) {
			auto NomeGrupo = (SText*)m_pControlContainer->FindControl(3741003 + i);
			NomeGrupo->SetTextColor(0xFFFFFFFF);
		}

		auto Desc2 = (SText*)m_pControlContainer->FindControl(3741011);
		Desc2->SetText(g_UIString[511], 0);

		auto Desc3 = (SText*)m_pControlContainer->FindControl(3741013);
		Desc3->SetText(g_UIString[513], 0);

		g_pClientInfo->Info.GrupoCheck = 0;

		auto GoButton = (SButton*)m_pControlContainer->FindControl(3741210);
		GoButton->SetText("Criar Grupo");
		break;
	}
	case SELECIONARGRUPO: {
		if (g_pClientInfo->Info.GrupoSelecionado == -1 || g_pClientInfo->Info.GrupoSelecionado != GrupoBtn) {
			g_pClientInfo->Info.GrupoSelecionado = GrupoBtn;
			...
			auto NomeGrupo = (SText*)m_pControlContainer->FindControl(3741003 + GrupoBtn);
			NomeGrupo->SetTextColor(TNColor::Speak);

			g_pClientInfo->Info.GrupoCheck = 0;

			auto Desc2 = (SText*)m_pControlContainer->FindControl(3741011);
			Desc2->SetText(g_UIString[510], 0);

			auto GoButton = (SButton*)m_pControlContainer->FindControl(3741210);
			GoButton->SetText("Entrar no Grupo");
			break;
		}
		if (g_pClientInfo->Info.GrupoSelecionado == GrupoBtn) {
			g_pClientInfo->Info.GrupoSelecionado = -1;
	...
	case 3741211: {
		if (g_pClientInfo->Info.GrupoCheck == 0) {
			g_pClientInfo->Info.GrupoCheck = 1;

			auto GoButton = (SButton*)m_pControlContainer->FindControl(3741211);
			GoButton->SetTextureSetIndex(-692);

			if (g_pClientInfo->Info.GrupoSelecionado != -1) {
				ReqAlias(45);
				g_pClientInfo->Info.GrupoPagina = 0;

				auto NomeGrupo = (SText*)m_pControlContainer->FindControl(3741003 + g_pClientInfo->Info.GrupoSelecionado);
				NomeGrupo->SetTextColor(0xFFFFFFFF);
		...
		if (g_pClientInfo->Info.GrupoCheck == 1) {
			g_pClientInfo->Info.GrupoCheck = 0;

			auto GoButton = (SButton*)m_pControlContainer->FindControl(3741211);
			GoButton->SetTextureSetIndex(-691);
	...
	case 3741200: {
		if (g_pClientInfo->Info.GrupoSelecionado != -1) {
			...
			auto GoButton2 = (SButton*)m_pControlContainer->FindControl(3741211);
			GoButton2->SetTextureSetIndex(-611);
	...
	case 3741210: { //Botão criar grupo/entrar
		if (g_pClientInfo->Info.GrupoSelecionado != -1) {
			auto GroupPainel = (SEditableText*)m_pControlContainer->FindControl(3741012);

			int Page = (g_pClientInfo->Info.GrupoPagina * 7) + g_pClientInfo->Info.GrupoSelecionado;

			SendTextServer("entrar", strFmt("%s %s", g_pClientInfo->Parties.Nick[Page], GroupPainel->GetText()));
			GroupPainel->SetText((char*)"");
			GroupPainel->SetFocused(0);
		}
		else {
			auto GroupPainel = (SEditableText*)m_pControlContainer->FindControl(3741012);

			if (g_pClientInfo->Info.GrupoCheck) {
				SendTextServer("grupo", strFmt("%s %s", "0000", GroupPainel->GetText()));
			}
			else {
				SendTextServer("grupo", strFmt("%s %s", "1111", GroupPainel->GetText()));
			}
	...
	case 3741208: {
		if (g_pClientInfo->Info.GrupoPagina > 0)
			g_pClientInfo->Info.GrupoPagina--;

		int Page = g_pClientInfo->Info.GrupoPagina * 7;

		for (int i = 0; i < 7; i++) {
			auto Cadeado = (SButton*)m_pControlContainer->FindControl(3741301 + i);
			Cadeado->SetTextureSetIndex(-2);
		}

		for (int i = 0; i < 7; i++) {
			auto NomeGrupo = (SText*)m_pControlContainer->FindControl(3741003 + i);

			if (g_pClientInfo->Parties.Index[i + Page] != 0) {
				auto Cadeado = (SButton*)m_pControlContainer->FindControl(3741301 + i);
				if (g_pClientInfo->Parties.Open[i + Page] == 1) {
					Cadeado->SetTextureSetIndex(692);
				}
				else {
					Cadeado->SetTextureSetIndex(691);
				}

				if (g_pClientInfo->Parties.Desc[i + Page][0] == 0) {
					NomeGrupo->SetText(strFmt("%s", g_pClientInfo->Parties.Nick[i + Page]), 0);
				}
				else {
					NomeGrupo->SetText(strFmt("%s", g_pClientInfo->Parties.Desc[i + Page]), 0);
				}
			}
			else
				NomeGrupo->SetText("", 0);
	...
	case 3741209: {
		if (g_pClientInfo->Info.GrupoPagina < 10)
			g_pClientInfo->Info.GrupoPagina++;
...
int TMFieldScene::OnPacketEvent(unsigned int dwCode, char* buf)
{
	...
	// FASE A: Packet entry validation
	if (!buf)
	{
		PV_LOG("OnPacketEvent: Null packet buffer received");
		return 1;
	}
	...
	{
		PV_LOG("OnPacketEvent: Packet size validation failed for opcode 0x%X", pStd->Type);
		return 1;
	}
	...
	{
		PV_LOG("OnPacketEvent: Unknown opcode 0x%X rejected", pStd->Type);
		return 1;
	}
#else
	auto pStd = (MSG_STANDARD*)buf;
	...
	case _MSG_SendPainelGuilda:
		return OnPacketSendPainelGuilda(pStd);
	case _MSG_SendParties:
		return OnPacketSendParties(pStd);
		break;
	}
...
int TMFieldScene::OnPacketSendParties(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendParties*>(pStd);
	memcpy(&g_pClientInfo->Parties, p, sizeof(MSG_SendParties));

	{
		for (int i = 0; i < 70; i++) {
			g_pClientInfo->Parties.Index[i] = p->Index[i];
			g_pClientInfo->Parties.Open[i] = p->Open[i];
			strncpy(g_pClientInfo->Parties.Nick[i], p->Nick[i], 16);
			strncpy(g_pClientInfo->Parties.Desc[i], p->Desc[i], 16);
		}
	}
	...
	for (int i = 0; i < 7; i++) {
		...
		if (p->Index[i] != 0) {
			...
			if (p->Desc[i][0] == 0) {
				NomeGrupo->SetText(strFmt("%s", p->Nick[i]), 0);
			}
			else {
				NomeGrupo->SetText(strFmt("%s", p->Desc[i]), 0);
			}
		}
		else
			NomeGrupo->SetText("", 0);
...
int TMFieldScene::OnPacketSendPainelGuilda(MSG_STANDARD* pStd)
{
	auto p = reinterpret_cast<MSG_SendPainelGuilda*>(pStd);
	{
		for (int i = 0; i < 4; i++) {
			auto Comps = (SGridControl*)m_pControlContainer->FindControl(57572034 + i);
	...
	}
	{
		for (int i = 0; i < 5; i++) {
			std::string sCoin;
			std::string sCoinF;
			sCoin = strFmt("%llu", p->Imposto[i]);
			sCoinF = quickAddThousandSeparators(sCoin);

			auto Imposto = (SText*)m_pControlContainer->FindControl(57572015 + i);
			Imposto->SetText(strFmt("%s", sCoinF.c_str()), 0);
		}

		auto Guilda = (SText*)m_pControlContainer->FindControl(57572004);
		Guilda->SetText(strFmt("Guild: %s", p->Guilda), 0);

		auto Id = (SText*)m_pControlContainer->FindControl(57572005);
		Id->SetText(strFmt("Guild Id: %d", p->Id), 0);

		auto Lider = (SText*)m_pControlContainer->FindControl(57572006);
		Lider->SetText(strFmt("Líder: %s", p->Líder), 0);

		auto Membros = (SText*)m_pControlContainer->FindControl(57572007);
		Membros->SetText(strFmt("Membros: %d", p->Membros), 0);

		auto MembrosOn = (SText*)m_pControlContainer->FindControl(57572008);
		MembrosOn->SetText(strFmt("Membros Online: %d", p->MembrosOn), 0);

		auto Fama = (SText*)m_pControlContainer->FindControl(57572009);
		Fama->SetText(strFmt("Fama: %d", p->Fama), 0);
	}
	return 1;
}
...
int TMFieldScene::FrameMove(unsigned int dwServerTime)
{
	...
	if (m_pRankTimeText && m_bRankTimeOn) //Greentime
	{
		if (m_pGround->m_vecOffsetIndex.x == 1 && m_pGround->m_vecOffsetIndex.y == 31
			...
			|| m_pGround->m_vecOffsetIndex.x == 18 && m_pGround->m_vecOffsetIndex.y == 16 // Coveiro
			|| m_pGround->m_vecOffsetIndex.x == 17 && m_pGround->m_vecOffsetIndex.y == 13 //Colheita
			|| m_pGround->m_vecOffsetIndex.x == 3 && m_pGround->m_vecOffsetIndex.y == 30 // Kaizen
			|| m_pGround->m_vecOffsetIndex.x == 5 && m_pGround->m_vecOffsetIndex.y == 29 // Mana
			|| m_pGround->m_vecOffsetIndex.x == 10 && m_pGround->m_vecOffsetIndex.y == 31 //Emblema
...
int TMFieldScene::MouseClick_NPC(int nX, int nY, D3DXVECTOR3 vec, unsigned int dwServerTime)
{
	...
	if (pOver->m_dwID >= 1000 && (pOver->m_stScore.Reserved & 0xF) == 1)
	{
		//NPC DE COMPOSIÇÃO
		for (int f = 0; f < 20; f++) {
			if (strcmp(pOver->m_szName, m_CompNames[f]) == 0) {
				auto Composer = (SPanel*)m_pControlContainer->FindControl(181921);
...
void TMFieldScene::OnESC()
{
	...
	else if (JanelaGrupo->IsVisible() == 1) {
		JanelaGrupo->SetVisible(0);
	}
	...
	else if (BattlePass->IsVisible() == 1) {
		BattlePass->SetVisible(0);
	}	
...
int TMFieldScene::OnPacketCreateMob(MSG_STANDARD* pStd)
{
	...
	if (pHuman)
	{
		...
		{
			sprintf(pHuman->m_TradeDesc, pHuman->m_szNickName);
			pHuman->m_pAutoTradeDesc->SetText(pHuman->m_TradeDesc, 0);
			pHuman->m_pNickNameLabel->SetText("", 0);

			pHuman->m_pNameLabel->SetText(pHuman->m_szName, 0);

			/*sprintf(pHuman->m_TradeDesc, );
			pHuman->m_pAutoTradeDesc->SetText(pHuman->m_TradeDesc, 0);
...
int TMFieldScene::OnPacketUpdateItemLevel(MSG_STANDARD* pStd)
{
	...
	if (g_pClientInfo->Info.chave1 == 2 && g_pClientInfo->Info.Keys[0] >= 113) {
		...
		int Index[5] = { 2372, 2367, 0, 0, 0 }; //Montaria Itemlevel
...
DWORD WINAPI Guildmark_Download(void* pArg)
{
	...
	strcpy(strURL, "http://supremewar.painelmmo.fun/guilds/"); //Guild Mark Link
