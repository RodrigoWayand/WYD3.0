	...
	DWORD cchDest, TCHAR* strDefault)
{
	...
	LONG lRes = RegQueryValueEx(hKey, strRegName, 0, &dwType, (BYTE*)strDest, &cbDest);

	// [CORREÇÃO] Proteção contra buffer insuficiente no registro
	...
	if (ERROR_SUCCESS != lRes)
	{
		...
		strDest[cchDest - 1] = 0;
		if (dwType != REG_SZ) return E_FAIL;
		return S_OK;
	}
	return E_FAIL;
}
...
VOID DXUtil_Trace(TCHAR* strMsg, ...)
{
	...
	_vsntprintf(strBuffer, 511, strMsg, args);
	strBuffer[511] = 0;
	va_end(args);
...
HRESULT CArrayList::Add(void* pEntry)
{
	...
	if (m_pData == NULL || m_NumEntries + 1 > m_NumEntriesAllocated)
	{
		...
		// proteção overflow
		if (NumEntriesAllocatedNew > 1000000)
	...
	{
		CopyMemory(
			(BYTE*)m_pData + (m_NumEntries * m_BytesPerEntry),
			pEntry,
			m_BytesPerEntry);
	}
	else
...
void CArrayList::Remove(UINT Entry)
{
	...
	MoveMemory(
		...
		(m_NumEntries - Entry) * m_BytesPerEntry);
}
