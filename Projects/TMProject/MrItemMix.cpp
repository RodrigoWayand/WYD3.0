                                ...
                                && m_dNeedRefer[i].dHavevolume < 1)
                            {
                                ...
                                for (int k = i; k < 8; ++k)
                                {
                                    if (!m_stCombineItem.Item[k].sIndex)
                                    {
                                        ...
                                        // [CORREÇÃO] Validação dos limites de CarryPos
                                        int carryPos = 15 * j + 5 * pItem->m_nCellIndexY + pItem->m_nCellIndexX;
                                        if (carryPos >= 0 && carryPos < 60) // 60 = MAX_CARRY
                                            m_stCombineItem.CarryPos[k] = carryPos;
                                        break;
                                    }
                                }
                            }
...
{
    // [CORREÇÃO] Proteção contra índice de item inválido
    if (!item || item->sIndex < 0 || item->sIndex >= MAX_ITEMLIST) // MAX_ITEMLIST = 4000 (ajuste conforme define do jogo)
        return 0;
...
void CItemMix::BASE_ReadMixList()
{
    ...
    // [CORREÇÃO] Verificar leitura
    size_t nRead1 = fread(stResult_itemList, sizeof(stResult_itemList), 1, fp);
    size_t nRead2 = fread(stNeed_itemList, sizeof(stNeed_itemList), 1, fp);
    if (nRead1 != 1 || nRead2 != 1)
    {
        MessageBoxA(0, "Mixlist.bin is corrupted", "ERROR", MB_OK);
    }
    fclose(fp);
}
...
int CItemMix::HardCode(SMessagePanel* MessagePanel, SGridControl** GridInvList)
{
    if (m_NPCHead == 54 && m_dNPCX == 19 && m_dNPCY == 13)
    {
        ...
        // [CORREÇÃO] Verificar Item2 antes de acessar
    ...
    else if (m_NPCHead == 55)
    {
        ...
        // [CORREÇÃO] Verificar se ambos os itens existem antes de usar
        if (!Item1 || !Item2)
        {
            MessagePanel->SetMessage(g_pMessageStringTable[412], 2000u);
            MessagePanel->SetVisible(1, 1);
    ...
    // ... (o restante da função permanece inalterado, mas vou mantê-lo igual ao original para não alongar)
    // Apliquei apenas as verificações de ponteiro nulo nos trechos mais críticos.
