...
int TMSky::Render()
{
    ...
    // Completely skip sky dome rendering when render override is active (eliminates spherical bubble/arc)
    if (g_pCurrentScene && g_pCurrentScene->m_pGround && 
        g_pCurrentScene->m_pGround->m_bRenderOverride &&
        (g_pCurrentScene->m_pGround->m_sRenderFlags & 0x0002) &&
        g_pCurrentScene->m_pGround->m_fRenderCullingRadius > 0.0f)
    {
        return 0;
    }
...
int TMSky::FrameMove(unsigned int dwServerTime)
{
    ...
    {
        g_pDevice->m_bFog = 0;
        g_pDevice->m_fFogStart = 0.0f;
        g_pDevice->m_fFogEnd = 10000.0f;
        g_pDevice->m_dwClearColor = 0xFF335599;
        if (g_ClipFar < 1000.0f)
            g_ClipFar = 1000.0f;
    }
    ...
    // Não resetar clip far quando render override está ativo
    if (!(g_pCurrentScene && g_pCurrentScene->m_pGround && 
          g_pCurrentScene->m_pGround->m_bRenderOverride &&
          (g_pCurrentScene->m_pGround->m_sRenderFlags & 0x0002) &&
          g_pCurrentScene->m_pGround->m_fRenderCullingRadius > 0.0f))
    ...
    if (g_pCurrentScene && g_pCurrentScene->m_pGround && g_pCurrentScene->m_pGround->m_bSkyOverride)
    {
        auto pGround = g_pCurrentScene->m_pGround;

        if ((pGround->m_sSkyFlags & 0x0004) && pGround->m_nSkyState >= 0 && m_nState != pGround->m_nSkyState)
            SetWeatherState(pGround->m_nSkyState);
    ...
    if (g_pCurrentScene && g_pCurrentScene->m_pGround && g_pCurrentScene->m_pGround->m_bRenderOverride)
    {
        auto pGround = g_pCurrentScene->m_pGround;
        if ((pGround->m_sRenderFlags & 0x0001) && pGround->m_fRenderClipFar > 0.0f)
        {
            float newClipFar = pGround->m_fRenderClipFar;
            if (fabsf(g_ClipFar - newClipFar) > 0.01f)
            {
                g_ClipFar = newClipFar;
                if (g_pDevice)
