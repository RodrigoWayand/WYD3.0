...
NewApp::NewApp()
{
	...
	m_pObjectManager = 0;
	m_pSocketManager = 0;
	m_pBlur = 0;               // [CORREÇÃO] inicializado
	g_pApp = this;
...
NewApp::~NewApp()
{
	// [CORREÇÃO] Garantir liberação de todos os recursos
	Finalize();
...
HRESULT NewApp::Initialize(HINSTANCE hInstance, int nFull)
{
	...
	m_dwColorBit = 16;
	m_bwFullScreen = 1; // FORĮADO FULLSCREEN
	...
	// [CORREÇÃO] validação robusta do índice
	if (nResIndex < 1 || nResIndex > 8)
		nResIndex = 3;
	...
	if (m_hWnd == nullptr)
	{
		...
		{
			m_dwWindowStyle = 0x80000000 | 0x10000000;
		}
		int winWidth = m_dwScreenWidth;
		int winHeight = m_dwScreenHeight;
		int winX = 0;
		int winY = 0;
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			if (m_bActive == 1)
			{
				...
				// [CORREÇÃO] Verificar event translator
...
HRESULT NewApp::MsgProc(HWND hWnd, DWORD uMsg, DWORD wParam, int lParam)
{
	...
	case WM_COMMAND:
	{
		...
		// [CORREÇÃO] Verificar cena antes de usar
		if (g_pCurrentScene != nullptr)
	...
	case WM_USER + 1:
	{
		...
		while (1)
		{
			...
			// [CORREÇÃO] Validação básica de tamanho
			if (pStd->Size < sizeof(MSG_STANDARD) || pStd->Size > 65536)
				break;
