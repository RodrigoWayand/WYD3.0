	...
	: TreeNode(0)
{
	...
	static int s_nControlCount = 0;
	if (s_nControlCount < 20)
	{
		LOG_WRITELOGSTRING("[SControl] #%d: Input(%.0f,%.0f,%.0f,%.0f) Screen=%dx%d Ratio=%.2f,%.2f Result(%.0f,%.0f,%.0f,%.0f)\r\n",
			s_nControlCount, inPosX, inPosY, inWidth, inHeight,
			g_pDevice->m_dwScreenWidth, g_pDevice->m_dwScreenHeight,
			fWidthRatio, fHeightRatio,
			m_nPosX, m_nPosY, m_nWidth, m_nHeight);
		s_nControlCount++;
	}
}
...
int SPanel::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	case WM_MOUSEMOVE:
	{
		if (m_bPicked != 0 && m_bPickable != 0)
		{
			...
			if ((float)(m_nPosX + m_nWidth) > (float)g_pDevice->m_dwScreenWidth)
				m_nPosX = (float)g_pDevice->m_dwScreenWidth - m_nWidth;
			if ((float)(m_nPosY + m_nHeight) > (float)g_pDevice->m_dwScreenHeight)
				m_nPosY = (float)g_pDevice->m_dwScreenHeight - m_nHeight;
...
int SButton::OnMouseEvent(unsigned int dwFlags, unsigned int wParam, int nX, int nY)
{
	...
	case WM_LBUTTONUP:
		...
		if (m_bEnable == 1 && m_bOver == 1 && m_bPressed == 1 && m_pEventListener != nullptr)
		{
			...
			{
				// [CORREÇÃO] Verificar se g_pSoundManager é válido antes de usar
				if (m_bSound == 1 && g_pSoundManager != nullptr)
				{
					CSound* pSound = g_pSoundManager->GetSoundData(53);
					if (pSound)
						pSound->Play(0, 0);
				}
...
{
	// [CORREÇÃO] Liberar todos os recursos para evitar vazamento
	SAFE_DELETE(m_pText);
	SAFE_DELETE(m_pText2);
	SAFE_DELETE(m_pPanelL);
	SAFE_DELETE(m_pPanelR);
}
...
void SMessagePanel::SetVisible(int bVisible, int bSound)
{
	...
	// [CORREÇÃO] Verificar se g_pSoundManager é válido antes de usar
	if (bVisible == 1 && bSound == 1 && g_pSoundManager != nullptr)
	{
		CSound* pSound = g_pSoundManager->GetSoundData(33);
		if (pSound)
			pSound->Play();
	}
}
...
int PointInRect(int inPosX, int inPosY, float ifX, float ifY, float ifWidth, float ifHeight)
{
	...
	int bResult = (float)inPosX >= ifX
		&& (float)inPosY >= ifY
		&& (float)(ifX + ifWidth) > (float)inPosX
		&& (float)(ifY + ifHeight) > (float)inPosY;

	// Log para debug de coordenadas do mouse (primeiros 100 testes)
	if (s_nLogCount < 100)
	{
		char szLog[256];
		sprintf(szLog, "[MouseDebug] Teste %d: Mouse(%d,%d) vs Rect(%.1f,%.1f,%.1f,%.1f) = %s\r\n",
			s_nLogCount, inPosX, inPosY, ifX, ifY, ifWidth, ifHeight, bResult ? "HIT" : "miss");
		LOG_WRITELOGSTRING(szLog);
		s_nLogCount++;
	}
