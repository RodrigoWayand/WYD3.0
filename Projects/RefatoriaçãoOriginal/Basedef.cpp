...
int BASE_GetRoute(int x, int y, int* targetx, int* targety, char* Route, int distance, char* pHeight, int MH)
{
    ...
    for (int i = 0; i < distance && i < 23; ++i)
    {
        ...
        int cul = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY) - g_HeightPosX];
        int n = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY - 1) - g_HeightPosX];
        int ne = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY - 1) - g_HeightPosX + 1];
        int e = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY) - g_HeightPosX + 1];
        int se = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY + 1) - g_HeightPosX + 1];
        int s = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY + 1) - g_HeightPosX];
        int sw = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY + 1) - g_HeightPosX - 1];
        int w = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY) - g_HeightPosX - 1];
        int nw = (signed char)pHeight[x + g_HeightWidth * (y - g_HeightPosY - 1) - g_HeightPosX - 1];

        int nMH = MH;
        if (cul >= -1 && cul <= 1 && (n < 0 || ne < 0 || e < 0 || se < 0 || s < 0 || sw < 0 || w < 0 || nw < 0))
            nMH = 30;
        if (tx == x && ty == y)
        {
            Route[i] = 0;
        ...
        if (tx == x && ty > y && s < nMH + cul && s > cul - nMH)
        {
            Route[i] = 56;
            ++y;
        }
        else if (tx == x && ty < y && n < nMH + cul && n > cul - nMH)
        {
            Route[i] = 50;
            --y;
        }
        ...
        else if (tx > x && ty == y && e < nMH + cul && e > cul - nMH)
        {
            Route[i] = 54;
            ++x;
        }
        ...
        else if (tx < x && ty == y && w < nMH + cul && w > cul - nMH)
        {
            Route[i] = 52;
            --x;
        }
        ...
        else if (tx > x && ty > y && s < nMH + cul && s > cul - nMH)
        {
            Route[i] = 56;
            ++y;
        }
